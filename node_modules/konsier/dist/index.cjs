...
function createKonsierSignature(input) {
  return (0, import_node_crypto.createHmac)("sha256", input.apiKey).update(`${input.timestamp}.${input.payload}`).digest("hex");
}
function verifyKonsierSignature(input) {
  ...
  try {
    ...
    if (!(0, import_node_crypto.timingSafeEqual)(expectedBuffer, providedBuffer)) {
      return { ok: false, reason: "SIGNATURE_MISMATCH" };
    }
    return { ok: true };
  } catch {
...
function verifyPageToken(input) {
  ...
  const expectedSignature = (0, import_node_crypto.createHmac)("sha256", input.apiKey).update(encodedPayload).digest("hex");
  try {
...
function createSignedPageToken(apiKey, payload) {
  ...
  const signature = (0, import_node_crypto.createHmac)("sha256", apiKey).update(encodedPayload).digest("hex");
...
function deriveInputSchema(toolName, schema) {
  ...
  if (isZodSchema(schema)) {
    try {
      const converted = (0, import_zod.toJSONSchema)(schema);
